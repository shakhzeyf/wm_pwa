(() => {
  ...
  // Extra mobile buttons (optional)
  function openPicker() {
    // Some browsers require direct user gesture; keep it synchronous.
    el.fileInput && el.fileInput.click();
  }
  if (el.btnOpenTop) el.btnOpenTop.addEventListener("click", openPicker);
  if (el.btnOpenBig) el.btnOpenBig.addEventListener("click", openPicker);
  if (el.btnOpenMob) el.btnOpenMob.addEventListener("click", openPicker);
