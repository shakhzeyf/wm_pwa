(() => {
  ...
  // Update status when libraries are loaded by index.html loader.
  window.addEventListener("wm:depsready", () => setStatus(STATUS_OK));
  window.addEventListener("wm:depsfail", () => setStatus(STATUS_FAIL));
  ...
  async function ensurePdfWorker(){
    ...
    if (urls[0]) window.pdfjsLib.GlobalWorkerOptions.workerSrc = urls[0];
    return urls[0] || "";
  }
  ...
  el.btnOpen.addEventListener("click", () => {
    // We use <label for="fileInput"> for better mobile support.
    // Reset value so selecting the same file again still triggers 'change'.
    try { el.fileInput.value = ""; } catch (e) {}
  });
  if (el.btnOpenTop) el.btnOpenTop.addEventListener("click", () => { try { el.fileInput.value = ""; } catch (e) {} });
  if (el.btnOpenBig) el.btnOpenBig.addEventListener("click", () => { try { el.fileInput.value = ""; } catch (e) {} });
  if (el.btnOpenMob) el.btnOpenMob.addEventListener("click", () => { try { el.fileInput.value = ""; } catch (e) {} });
  ...
  el.fileInput.addEventListener("change", async () => {
    ...
    try {
      ...
      if (file.type === "application/pdf" || file.name.toLowerCase().endsWith(".pdf")) {
        ...
        await ensurePdfWorker();
        pdfDoc = await window.pdfjsLib.getDocument({ data: pdfBytes }).promise;
